...
def imageLoader(img_filenames,depth_filenames,depth_skip,seq_len=3):
    ...
    while True:
        ...
        while batch_start < numSamples:
            ...
            img_batch = np.concatenate(list(map(load_and_scale_image,img_filenames[batch_start:limit],itertools.repeat(random_index,len(img_filenames[batch_start:limit])))),0)
            depth_batch = np.concatenate(list(map(load_and_scale_depth,depth_filenames[batch_start:limit],itertools.repeat(random_index,len(depth_filenames[batch_start:limit])))),0)
